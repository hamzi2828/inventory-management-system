                                                    ...
                                                    <td>
                                                        <div class="d-flex">
                                                            <a class="btn btn-dark btn-sm d-block mb-25 me-25"
                                                               target="_blank"
                                                               href="{{ route ('adjustment-decrease-invoice', ['stock_return' => $return -> id]) }}">
                                                                Print
                                                            </a>
                                                            
                                                            @can('editAdjustmentsDecrease', \App\Models\User::class)
                                                                <a class="btn btn-primary btn-sm d-block mb-25 me-25"
                                                                   href="{{ route ('adjustments.edit-adjustment-decrease', ['stock_return' => $return -> id]) }}">
                                                                    Edit
                                                                </a>
                                                            @endcan
                                                            
                                                            @can('deleteAdjustmentsDecrease', \App\Models\User::class)
                                                                <form method="post"
                                                                      id="delete-confirmation-dialog-{{ $return -> id }}"
                                                                      action="{{ route ('adjustments.delete-adjustment-decrease', ['stock_return' => $return -> id]) }}">
                                                                    @method('DELETE')
                                                                    @csrf
                                                                    <button type="button"
